...
def get_predicted_value(patient_symptoms):
    ...
    send_pushover_notification(f" Detected with the following disease {diseases_list[svc.predict([input_vector])[0]]} ")
    ...
    if request.method == 'POST':
        ...
        if symptoms =="Symptoms":
            ...
            result = (
                ...
                + "\n".join(f"{i}: {p_i}" for i, p_i in enumerate(precautions[0], 1)) + "\n\n"

                "___________________Medications__________________\n"
                + "\n".join(f"{i}: {m_i}" for i, m_i in enumerate(medications, 1)) + "\n\n"

                "___________________Workout__________________\n"
                + "\n".join(f"{i}: {w_i}" for i, w_i in enumerate(workout, 1)) + "\n\n"

                "___________________Diet__________________\n"
                + "\n".join(f"{i}: {d}" for i, d in enumerate(rec_diet, 1))
)


                
            # send_pushover_notification(f" Followning Precation{dis_des}{precautions}{medications}{rec_diet}{workout} ")
            send_pushover_notification(result)
